...
function TraderProfileContent({ trader: initialTrader }: { trader: Trader }) {
...
export default function TraderPage() {
    ...
    useEffect(() => {
        if (traderId) {
            const traderData = getTraderDataById(traderId);
            setTrader(traderData);
        }
    }, [traderId]);

    if (!trader) {
        return <div className="flex items-center justify-center min-h-screen">Loading trader profile...</div>;
    }

    return <TraderProfileContent trader={trader} />;
}
